...
const ManagerDashboard = () => {
    ...
    return (
        <div className="main-content">
                    ...
                    <>
                        <Card className="border-l-4 border-l-accent shadow-xl shadow-accent/5">
                            ...
                            <div className="flex items-center gap-2 mt-4">
                                <span className="tag tag-success">Target Met</span>
                                <span className="text-[9px] text-slate-400 uppercase font-black tracking-widest">92% of Quota</span>
                            </div>
                        </Card>
                        <Card className="border-l-4 border-l-emerald shadow-xl shadow-emerald/5">
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Total Concentrate Yield</p>
                            <div className="stat-value text-slate-900">{dashboardStats.totalConcentrate.toFixed(1)} <span className="text-sm font-normal text-slate-400">TONS</span></div>
                            <div className="flex items-center gap-2 mt-4">
                                <span className="tag tag-warning">Monitoring</span>
                                <span className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Efficiency -1.2%</span>
                            </div>
                        </Card>
                        <Card className="border-l-4 border-l-primary shadow-xl shadow-primary/5">
                            ...
                            <div className="flex items-center gap-2 mt-4">
                                ...
                                <span className="text-[9px] text-slate-400 uppercase font-black tracking-widest">Industy: 12.8%</span>
                            </div>
                        </Card>
                    </>
                )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '2.5rem', marginBottom: '2rem' }}>
                {/* ... Trend & Ledger Cards ... */}
                <Card title="Production Trend (Last 7 Days)">
                    ...
                    {isLoading ? (
                        ...
                        <div className="h-56 w-full p-4 bg-slate-900/5 rounded-2xl border border-slate-100 flex flex-col justify-between">
                    ...
                    {isLoading ? (
                        ...
                        <div className="table-wrapper">
                            <table>
                                ...
                                <tbody>
                                    {['Wolframite (ROM)', 'Tungsten Concentrate', 'Waste Rock', 'Tailings'].map(m => {
                                        const logs = productionLogs.filter(l => l.mineral === m);
                                        const avg = logs.length > 0 ? (logs.reduce((s, l) => s + l.quantity, 0) / logs.length).toFixed(1) : 0;
                                        return (
                                            <tr key={m}>
